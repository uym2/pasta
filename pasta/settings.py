...
class PastaUserSettings(UserSettingsContainer):
    def __init__(self):
        ...
        self.sate.add_option('max_subtree_brlen', FloatUserSetting(name='max_subtree_brlen_frac', default=0, min=0, max=1, short_name=None, help='Can only be used with "break_constraint=brlen". The maximum total branch length of each subtree (subproblem) in proportion to the total branch length of the tree.  When the (sum branch length of a subtree / sum branch length of the tree) is less than or equal to this number, it will not be decomposed further. [default: max_subproblem_size/n where n is the number of leaves]', subcategory='decomposition'))
